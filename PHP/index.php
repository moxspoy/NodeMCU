	...
	*/
	...
	if(isset($pass) && ($pass == $passcode)){
		...
		if(isset($s1)&&isset($s2)){
			...
			$sql = "INSERT INTO test (date, s1, s2)
			VALUES ('$date', '$s1', '$s2')";
...
<html>
	...
	<body>
		<?php	
			...
			$result = mysqli_query($conn, "SELECT id, date, s1, s2 FROM iot ORDER BY id DESC LIMIT " . $display . "");
			...
			$row_cnt = mysqli_num_rows(mysqli_query($conn, "SELECT date FROM iot"));
